...
export default class HasMany extends Relation {
  ...
  onCreateWithRelated (data, related) {
    if (!Array.isArray(related) || related.find((_related) => !_.isPlainObject(_related))) {
      throw new InvalidCreateError(`hasMany relation "${this.fieldname}" must be an array of objects`)
    }
  ...
  set (instance, value) {
    if (
      ...
      throw new InvalidUpdateError(`${this.fieldname} requires an array of ${this.RelatedEntity.name}`)
    }
